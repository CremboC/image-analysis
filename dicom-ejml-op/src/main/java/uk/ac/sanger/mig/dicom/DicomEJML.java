...
public class DicomEJML {
	...
	private Matrix moments(File file, boolean findCenterAxis) throws IOException {
		...
		Matrix originalImage = imageMatrix.clone();
	 ...
	 * @return
	 */
	private Matrix rotate(Matrix matrix, Point p) {
		long startTime = System.currentTimeMillis();

		Matrix imageMatrix = matrix;
		Matrix rotatedMatrix = null;

		double angle = MathHelper.angleToHor(p);

		// create the (affine) rotation matrix [cos a, -sin a, 0; sin a, cos a, 0; 0, 0, 1]
		...
		// create a last coord matrix, must be affine [x y 1]
		Matrix lastCoord = Matrix.factory.zeros(3, 1);
		lastCoord.setAsInt((int) cols, 0, 0);
		lastCoord.setAsInt(0, 1, 0);
		lastCoord.setAsInt(1, 2, 0);
		lastCoord = rotationMatrix.mtimes(Ret.NEW, true, lastCoord);
		...
		// compensate for moving the whole image
		side += p.x + 1000;
		...
		rotatedMatrix = MatrixHelper.transform(imageMatrix, rotationMatrix, rotatedMatrix, p);

		// reduces matrix physical size
		rotatedMatrix = MatrixHelper.removeFalseRowsAndColumns(rotatedMatrix, (long) side);

		// fill empty pixels after transformation
		rotatedMatrix = MatrixHelper.fillEmptyPixels(rotatedMatrix, 2);
